...
//The functions stored here all belong to the class coulombInt. 
...
double coulombInt::Coulomb_HO(double &hw, int &ni, int &mi, int &nj, int &mj, int &nk, int &mk, int &nl, int &ml)
{
//This function accepts the oscillator frequency in atomic units, along with the sets of n and m quantum numbers for 
//four orbitals, and computes the Coulomb two-body interaction integral. 
...
double coulombInt::logfac(int &n)
{
  if(n < 0){ std::cerr << n << " : LogFactorial input should be >= 0" << std::endl; exit(1); }
  double fac = 0.0;
  for(int a = 2; a < n+1; a++){ fac += std::log(a); }
  return fac;
}

double coulombInt::logratio1(int &int1, int &int2, int &int3, int &int4)
{
  return -logfac(int1) - logfac(int2) - logfac(int3) - logfac(int4);
}

double coulombInt::logratio2(int &G)
{
  return -0.5 * (G + 1) * log(2);
}

double coulombInt::product1(int &n1, int &m1, int &n2, int &m2, int &n3, int &m3, int &n4, int &m4)
{
...
double coulombInt::logproduct2(int &n1, int &m1, int &n2, int &m2, int &n3, int &m3, int &n4, int &m4, int &j1, int &j2, int &j3, int &j4)
{
  int arg1 = n1 + std::abs(m1);
  int arg2 = n2 + std::abs(m2);
  int arg3 = n3 + std::abs(m3);
...
double coulombInt::logproduct3(int &l1, int &l2, int &l3, int &l4, int &g1, int &g2, int &g3, int &g4)
{
  int garg1 = g1 - l1;
  int garg2 = g2 - l2;
  int garg3 = g3 - l3;
